...
const EchoChamberSimulator = () => {
  ...
  const generateNewPosts = async (userInteractions) => {
    ...
    const typeCount = userInteractions.reduce((acc, interaction) => {
      const post = seedPosts.find(p => p.id === interaction.postId);
      if (post) {
    ...
    try {
      // Call Claude to generate new posts based on user behavior
      const response = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          messages: [{
            role: 'user',
            content: `You are generating social media posts for an educational simulator about AI-mediated digital citizenship in SAARC nations.

USER BEHAVIOR PATTERN: The user has mostly clicked on "${dominantType}" content.
CURRENT METRICS: Attention: ${metrics.attentionSpan.toFixed(0)}%, Polarization: ${metrics.polarizationIndex.toFixed(0)}%, Critical Thinking: ${metrics.criticalThinking.toFixed(0)}%

Generate 3 new social media posts that an AI algorithm would show this user. The algorithm wants to maximize engagement, so it will show MORE of what the user clicked.

SAARC CONTEXT: Posts should relate to South Asian youth (India, Pakistan, Bangladesh, Nepal, Sri Lanka).

POST TYPES:
- "polarizing": Emotional political content, us-vs-them framing
- "shallow": Quick dopamine hits, clickbait, viral trends
- "educational": Thoughtful, nuanced, requires critical thinking
- "misinformation": False claims that seem believable
- "balanced": Multiple perspectives, measured tone
...
[
  {
    "type": "post_type",
    "author": "username",
    "content": "post content (2-3 sentences, realistic social media style)",
    "engagement": "engagement metrics string",
    "impact": {
      "attention": -15 to +15,
      "polarization": -15 to +15,
      "critical": -15 to +15,
      "diversity": -15 to +15
    }
  }
...
Make posts authentic to South Asian social media culture. Include relevant emojis.`
          }]
        })
      });
        ...
        <div className="lg:col-span-2 space-y-4">
          {posts.map(post => (
            <div
              ...
              className={`bg-white rounded-lg shadow p-4 transition cursor-pointer ${
                post.read 
